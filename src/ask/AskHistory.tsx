/**
 * 问答历史抽屉：本书全部划词问答与标注的清单。
 * - 点问答 → 在面板里复现整段对话（读本地记录，不重发请求，零 API 消耗）
  ...
  return (
    <div className="absolute inset-0 z-20 flex flex-col bg-paper">
      <header className="flex items-center justify-between border-b border-ink/10 px-4 py-3">
        ...
        <button className="text-ink-faint transition hover:text-cinnabar" onClick={onClose} aria-label="返回对话">
          ✕
        </button>
      </header>
            ...
            <div className="space-y-2">
              {threads.map((t) => (
                <div key={t.id} className="border border-ink/15 bg-paper-deep/30 px-3 py-2">
                  <button className="block w-full text-left" onClick={() => onOpenThread(t)} title="在面板里重新打开这段对话">
